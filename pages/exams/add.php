...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // If there are no errors, add the exam
    if (empty($errors)) {
        ...
        $insert_query = "INSERT INTO exams (title, subject_id, exam_date, description, location) 
                         VALUES (?, ?, ?, ?, ?)";
                         
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param('sisss', $title, $subject_id, $datetime, $description, $location);
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="time" class="form-control" id="exam_time" name="exam_time" 
                                   value="<?php echo htmlspecialchars($_POST['exam_time'] ?? ''); ?>">
